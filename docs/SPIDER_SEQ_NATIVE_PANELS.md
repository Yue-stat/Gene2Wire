...
Run [`notebooks/native_target_panels/spider_seq.ipynb`](../notebooks/native_target_panels/spider_seq.ipynb)
in the existing OnDemand environment.
This uses the authors' scRNA-seq `Adult.Ex.rds`, not spatial `sp.PFC.rds`.
The canonical notebook pins its own shared core; earlier releases remain under
`archive/notebooks/`.
...
customized for SPIDER-Seq. The companion
[`notebooks/target_block_masking/spider_seq.ipynb`](../notebooks/target_block_masking/spider_seq.ipynb)
