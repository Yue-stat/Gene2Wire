...
OUTPUT_DIR = REPO_ROOT / "notebooks" / "native_target_panels"
spec = importlib.util.spec_from_file_location(
    "_notebook_common", Path(__file__).with_name("build_positive_label_hiding.py"))
    ...
    return {'cells': [common.cell(kind, source, i, date_suffix) for i, (kind, source) in enumerate(parts)],
        'metadata': {'kernelspec': {'display_name': 'Python 3 (OnDemand)', 'language': 'python', 'name': 'python3'},
            ...
            'gene2wire': {'core_commit': commit, 'source_hash': source_hash,
                'notebook_date': date_suffix, 'dataset': 'SPIDER-Seq',
                'protocol': common.PROTOCOL_VERSION, 'experiment': 'native_panels'}},
        'nbformat': 4, 'nbformat_minor': 5}


def build(commit, source_hash, output_dir=OUTPUT_DIR, date=None):
    date = common.release_date(date)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / 'spider_seq.ipynb'
    path.write_text(json.dumps(notebook(commit, source_hash, date),
...
if __name__ == '__main__':
    ...
    print(build(args.commit, args.source_hash, args.output_dir, args.date))
