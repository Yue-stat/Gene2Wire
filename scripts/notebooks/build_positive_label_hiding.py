"""Build the canonical positive-label-hiding OnDemand entry points.

Usage: python scripts/notebooks/build_positive_label_hiding.py \
    --commit <SHA> --source-hash <SHA256> [--date MMDD]

Canonical filenames are stable. The release date is retained inside each notebook
for provenance and figure names; historical releases live under ``archive/``.
...
def notebook(name, commit, source_hash, date_suffix=None):
    ...
    return {"cells": [cell(kind, source, i, date_suffix) for i, (kind, source) in enumerate(parts)],
            "metadata": {"kernelspec": {"display_name": "Python 3 (OnDemand)", "language": "python", "name": "python3"},
                         ...
                         "gene2wire": {"protocol": PROTOCOL_VERSION, "core_commit": commit,
...
def build(commit="__CORE_COMMIT_0908__", source_hash="__SOURCE_HASH_0908__", output_dir=OUTPUT_DIR,
          date_suffix=None):
    date_suffix = release_date(date_suffix)
