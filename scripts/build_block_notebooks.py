"""Build independent dated OnDemand group-by-target block experiment notebooks.

No model, mask, or fitting implementation lives in these entry points.
Usage: python scripts/build_block_notebooks.py --commit SHA --source-hash SHA256
"""
...
def main():
    ...
    date = _common.release_date(args.date)
    for name in NAMES:
        path = ROOT / f"{name}_block_masking_{date}.ipynb"
        path.write_text(json.dumps(notebook(name, args.commit, args.source_hash, date),
