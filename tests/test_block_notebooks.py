...
NOTEBOOK_DIR = ROOT / "notebooks" / "target_block_masking"
...
def builder():
    spec = importlib.util.spec_from_file_location(
        "block_notebook_generator",
        ROOT / "scripts" / "notebooks" / "build_target_block_masking.py",
    )
...
def test_checked_in_notebooks_use_stable_paths_and_are_clean(name):
    assert builder().NOTEBOOK_FILENAMES == NOTEBOOK_FILENAMES
    notebook = json.loads((NOTEBOOK_DIR / NOTEBOOK_FILENAMES[name]).read_text())
